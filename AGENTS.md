...
│   │   │   ├── chat/
│   │   │   │   ├── hooks.ts    # useProjectChat hook
│   │   │   │   └── system-prompt.ts
│   │   │   └── trpc/           # tRPC client/server setup
│   │   └── server/routers/     # tRPC routers (project, logo, chat, generation, export, subscription)
│   ├── prisma/
...
│   ├── nanobanana/             # Gemini image generation skill
│   └── logo-creator/          # Logo workflow orchestration skill
...
- **Deploy**: `npx vercel --prod` from `web/` directory
...
Prisma 7 requires `prisma.config.ts` at `web/` root — do NOT delete it.
